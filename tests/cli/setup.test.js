...
test('ready ai --json sees provider from a 0600 ai.env without sourcing', async t => {
  const env = environment(t);
  const settings = path.join(env.HOME, '.config', 'vibium', 'ai.env');
  fs.mkdirSync(path.dirname(settings), { recursive: true });
  fs.writeFileSync(settings, [
    'export VIBIUM_AI_PROVIDER=openai-compatible',
    'export VIBIUM_AI_MODEL=fixture-model',
    'export VIBIUM_AI_BASE_URL=http://127.0.0.1:1/v1',
  ...
  assert.equal(body.result.checks.find(c => c.name === 'VIBIUM_AI_MODEL').status, 'passed');
  assert.doesNotMatch(result.stdout + result.stderr, /dummy-key|source ~\/\.config\/vibium\/ai\.env|does not load/);
});

test('world-readable ai.env is not loaded', async t => {
  const env = environment(t);
  const settings = path.join(env.HOME, '.config', 'vibium', 'ai.env');
  fs.mkdirSync(path.dirname(settings), { recursive: true });
  fs.writeFileSync(settings, 'VIBIUM_AI_PROVIDER=openai\nVIBIUM_AI_MODEL=x\nOPENAI_API_KEY=secret-file-marker\n', { mode: 0o644 });
  const result = await run(env, ['ready', 'ai', '--json']);
  assert.equal(result.code, 1);
  assert.match(result.stderr, /chmod 0600/);
  assert.doesNotMatch(result.stdout + result.stderr, /secret-file-marker/);
  const body = JSON.parse(result.stdout);
  assert.equal(body.result.checks.find(c => c.name === 'VIBIUM_AI_PROVIDER').status, 'failed');
  assert.ok(body.result.notes.some(n => n.includes('chmod 0600')), body.result.notes);
});
