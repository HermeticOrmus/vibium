...
func TestReadyEnvNoteNoFilePointsAtSetup(t *testing.T) {
	_ = setupTestEnv(t)
	notes := readyEnvNote()
	if len(notes) != 1 || !strings.Contains(notes[0], "vibium setup") || !strings.Contains(notes[0], "config init") {
		t.Fatalf("notes: %#v", notes)
	}
	if strings.Contains(notes[0], "source ") {
		t.Fatal("missing file should not tell the user to source")
	}
}

func TestReadyEnvNoteSkipsSourceWhenProviderLoaded(t *testing.T) {
	home := setupTestEnv(t)
	path := filepath.Join(home, ".config", "vibium", "ai.env")
	if err := os.MkdirAll(filepath.Dir(path), 0o700); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(path, []byte("export VIBIUM_AI_PROVIDER=openai\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	t.Setenv("VIBIUM_AI_PROVIDER", "openai")
	notes := readyEnvNote()
	for _, n := range notes {
		if strings.Contains(n, "source ") || strings.Contains(n, "does not load") {
			t.Fatalf("note still asks to source: %q", n)
		}
	}
}

func TestReadyEnvNoteWorldReadable(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("unix modes")
	}
	home := setupTestEnv(t)
	...
	notes := readyEnvNote()
	if len(notes) != 1 || !strings.Contains(notes[0], "chmod 0600") {
		t.Fatalf("notes: %#v", notes)
	}
	if envfile.OwnerOnly(0o644) {
		t.Fatal("0644 must not be owner-only")
	}
}
